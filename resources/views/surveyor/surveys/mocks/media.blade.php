...
<div class="survey-media-mock-content">
    ...
    <div class="survey-media-mock-header-bar">
        <div class="survey-media-mock-header-left">
            <span class="survey-media-mock-address">{{ $survey->full_address ?? '123, Sample Street, Kent DA9 9ZT' }}</span>
        </div>
        <div class="survey-media-mock-header-right">
            ...
            <div class="survey-media-mock-header-icons">
                ...
                <button type="button" class="survey-media-mock-icon-btn" id="survey-media-mock-profile-btn" title="Profile">
        ...
        <div class="survey-media-mock-upload-section" id="upload-section">
            ...
            <div class="survey-media-mock-file-queue" id="file-queue" style="display: none;">
                <div class="survey-media-mock-file-queue-header">
                    ...
                    <button type="button" class="survey-media-mock-clear-queue-btn" id="clear-queue-btn">Clear All</button>
                </div>
            ...
            <div class="survey-media-mock-upload-actions">
                <button type="button" class="survey-media-mock-upload-btn" id="upload-files-btn" style="display: none;">
                    Upload Files
                </button>
            </div>
        </div>
...
$(document).ready(function() {
    ...
    const uploadUrl = "{{ route('surveyor.survey.media.upload', $survey) }}";
    ...
    function handleFiles(files) {
        ...
        if (invalidFiles.length > 0) {
            alert(`Some files exceed the maximum size of 100MB:\n${invalidFiles.map(f => f.name).join('\n')}`);
            return;
        }
    ...
    function uploadFileToS3(item, index) {
        ...
        // Laravel expects files as 'files[]' for array or 'files' for single file
        // Since we're uploading one at a time, use 'files[]' format
            ...
            if (xhr.status === 200) {
                try {
                    ...
                    if (response.success && response.uploaded_files && response.uploaded_files.length > 0) {
                        // Since we upload one file per request, get the first uploaded file
                        // Try to match by original name, otherwise just take the first one
                        let uploadedFile = response.uploaded_files.find(f => 
                            f.original_name === item.name
                        );
                        
                        // If no match found, take the first uploaded file
                        if (!uploadedFile && response.uploaded_files.length > 0) {
                            uploadedFile = response.uploaded_files[0];
                        }
                        
                        if (uploadedFile) {
                            item.status = 'completed';
                            item.metadata = uploadedFile;
                            updateProgressBar(index, 100, 'completed');
                            
                            // Store metadata in data attribute for future transcription
                            ...
                            progressItem.attr('data-needs-transcription', uploadedFile.needs_transcription || false);
                            ...
                            item.status = 'error';
                            showUploadError(index, 'File not found in server response');
                        }
                    } else {
                        item.status = 'error';
