...
@php
    ...
    $statusesList = collect(['pending', 'in_progress', 'completed', 'late', 'assigned', 'cancelled'])->filter(function ($st) use ($surveys) {
...
@section('content')
    ...
    <div class="survey-table-section">
        <x-datatable id="surveysTable" :columns="['Address', 'Client', 'Level', 'Status', 'Survey Date', 'Due Date', 'Surveyor', 'Job Reference']" :search="false" :filter="false" :clickableRows="true"
            rowDataAttribute="data-href">
                            @forelse($surveys as $survey)
                <tr class="clickable-row" data-href="{{ route('admin.surveys.show', $survey->id) }}">
                    <td>{{ Str::limit($survey->full_address ?? $survey->property_address_full, 60) }}{{ $survey->postcode ? ', ' . $survey->postcode : '' }}</td>
                    <td>{{ $survey->client_name ?? 'N/A' }}</td>
                    <td>
                    ...
                    <td>{{ $survey->job_reference ?? '' }}</td>
                            ...
                            @empty
                                <tr>
                    <td colspan="8" class="text-center py-5 no-data">No survey applications yet.</td>
                                </tr>
                            @endforelse
        </x-datatable>
</div>
...
<script>
        $(document).ready(function() {
            ...
            $(document).on('datatable:initialized', function(e, table, tableId) {
                ...
                // Status filter - column index 3 (Status)
                if (statusSelect) {
                ...
                // Level filter - column index 2 (Level)
                if (levelSelect) {
